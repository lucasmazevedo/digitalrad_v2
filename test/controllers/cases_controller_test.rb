...
class CasesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create case" do
    assert_difference('Case.count') do
      post cases_url, params: { case: { content: @case.content, status: @case.status, title: @case.title } }
    end
  ...
  test "should update case" do
    patch case_url(@case), params: { case: { content: @case.content, status: @case.status, title: @case.title } }
