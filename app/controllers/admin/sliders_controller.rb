class Admin::SlidersController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @slider.save
        ...
        format.json { render :show, status: :created, location: [:admin, @slider] }
      else
  ...
  def update
    respond_to do |format|
      if @slider.update(slider_params)
        ...
        format.json { render :show, status: :ok, location: [:admin, @slider] }
      else
  ...
  private
    ...
    def set_slider
      @slider = Slider.friendly.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def slider_params
      params.require(:slider).permit(:title, :subtitle, :link, :hero_image, :slug, :status)
    end
end
