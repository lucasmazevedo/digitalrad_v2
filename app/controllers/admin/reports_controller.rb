class Admin::ReportsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @report.save
        ...
        format.json { render :show, status: :created, location: [:admin, @report] }
      else
  ...
  def update
    respond_to do |format|
      if @report.update(report_params)
        ...
        format.json { render :show, status: :ok, location: [:admin, @report] }
      else
  ...
  private
    ...
    def report_params
      params.require(:report).permit(:title, :content, :hero_image, :slug, :status)
    end
end
